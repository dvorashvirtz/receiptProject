...
describe('MongoOperations', () => {
    ...
    it('find should filter a item from a collection', async () => {
        let mongo1 = new MongoOprations(MONGO_RECEIPT_DB);
        const client = getClient();
        mongo1.myCollection = client.db(MONGO_RECEIPT_DB).collection(MONGO_CUSTOMERS_COLLECTION);
        const name = 'david';
        const result = await mongo1.find({ filter: { name } });
        expect(result).toBeDefined();
